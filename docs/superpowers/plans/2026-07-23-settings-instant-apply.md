...
- 默认设置仅为快捷键 `Shift+Space` 与开机启动 `false`。
...
it('persists autostart immediately and confirms with the authoritative snapshot', async () => {
  ...
  expect(client.saveSettings).toHaveBeenCalledWith({
    settings: { hotkey: 'Shift+Space', autostart: true },
  })
...
async function resetSettings(): Promise<void> {
  ...
  publish(true)
  await persistSettings({ hotkey: 'Shift+Space', autostart: false })
}
```
...
**Interfaces:**
- Produces: `Settings::default().hotkey == "Shift+Space"`。
- Produces: `LauncherCore.resetSettings()` 固定提交 `{ hotkey: "Shift+Space", autostart: false }`。
- Preserves: 保存与 authoritative load 期间短暂只读；成功后恢复 `ready` 且允许 `saveHotkeyCanonical()`。

- [ ] **Step 1: 写默认值与恢复解锁 RED 测试**

在 Rust settings 测试中直接断言：
...
在 React 设置页测试中把恢复提示和 payload 期望改为 `Shift+Space`；模拟恢复成功后的 authoritative load 返回 `Shift+Space`，断言 `readOnly == false`，随后在快捷键录制器完成一次有效录制并断言 `saveHotkey` 被调用。

- [ ] **Step 2: 运行 RED**

Run: `cargo test settings::tests::default_hotkey_is_shift_space --manifest-path .\src-tauri\Cargo.toml -- --exact`

Expected: FAIL，当前默认值为 `Alt+Space`。

Run: `npm.cmd test -- --run src/launcher.test.tsx -t "resets settings only after confirmation"`

Expected: FAIL，当前提示和 reset payload 仍为 `Alt+Space`。

- [ ] **Step 3: 实现最小默认值修订**

将 `Settings::default().hotkey`、`resetSettings()` 的乐观值与固定 payload、Popconfirm 说明统一为 `Shift+Space`。不改变 settings operation、authoritative load 或 `settingsUncertain` 生命周期。
...
只提交上述产品代码、测试与本次文档修订；保留 worktree，等待人工验证 `Shift+Space` 初始化、恢复成功解锁和再次录制。
