...
- [ ] On a failure before durable commit, `abort_before_commit` must publish the still-installed plugin under a new epoch and return a new `PublicRuntimeCandidate`. The command destroys the old Runtime/window, creates the replacement Runtime through the existing service readiness path, and only then reports recoverable failure. If replacement creation/readiness fails, apply the existing `mark_runtime_unavailable`/fault-disable contract; never leave an enabled plugin with no Runtime and no fault state. After durable commit, never restore the plugin; return `PublicPluginManagementError::DataCleanupPending` / `CommandError.code = dataCleanupPending`.
