...
本地计数不记录查询词、应用名称或文件路径。试用者每周主动导出本地聚合数据，并参加一次不超过 20 分钟的访谈。导出文件必须包含研究参与者 ID，因此属于可关联的假名化数据，不称为匿名数据。设置中该 ID 可暂时为空；非空时必须为 1 到 64 个 ASCII 字母、数字、`-` 或 `_`。加载和保存统一拒绝其他格式，用户确认导出后若仍未配置，导出返回固定 `MissingResearchId` 错误且不创建文件。
