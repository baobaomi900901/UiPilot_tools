...
- 结构化查询 API 不可用或返回错误时显示索引错误，不回退到字符串拼接或磁盘遍历。
...
- `file_debounce_elapsed`：150ms 防抖完成，主 WebView 准备发送文件查询。
...
- 文件搜索防抖：`query_input` 到 `file_debounce_elapsed` 不早于 150ms，200 条固定查询的 P95 不超过 200ms。
- 文件搜索宿主开销：`file_debounce_elapsed` 到 `file_query_submitted`，200 条固定查询，P95 不超过 100ms。
