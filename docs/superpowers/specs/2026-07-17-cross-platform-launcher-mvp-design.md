...
一次文件搜索会话从输入 `/find` 开始，到执行结果、清空查询或隐藏窗口时结束；会话内的每次按键不重复计数。结果为空或缺少预期文件时，用户可以点击“未找到预期文件”，每个会话最多计数一次，不保存查询词或路径。
...
- 文件覆盖率代理 = `1 - 未找到预期文件次数 / 文件搜索会话次数`。
- 用户每周以 1 至 5 分评价“文件搜索是否覆盖了我预期能找到的文件”。

文件搜索会话次数为 0 时，H4 视为未获得验证，不能满足 Go 条件。
...
- Rust 为每次查询生成 `requestId`，并在内存中维护 `requestId + resultId -> 真实动作与目标` 映射。
...
- `first_file_result_received`：Rust 从 SystemIndex 收到首个结果。
...
- SystemIndex 等待：`file_query_submitted` 到 `first_file_result_received`，200 条保证命中的固定查询，单独报告 P50、P95 和最大值。
- 文件结果 IPC/渲染开销：`first_file_result_received` 到 `first_file_result_committed`，同一批 200 条查询，P95 不超过 100ms。
