...
**Status:** Draft - second independent review findings addressed, awaiting re-review
...
- Manifest fatal UTF-8, duplicate-key, JSON syntax, isolated/reversed surrogate escapes in nested keys and values, `uint32`/`double` Ajv formats, Schema, and semantic validation.
...
- Manifest strings with isolated or reversed surrogate escapes are rejected before Ajv.
