...
`.result-list` 使用 WebView2 支持的原生 CSS 滚动条伪元素呈现细窄浮层观感：

- 滚动条宽度固定为 `6px`，轨道透明，不增加外框或背景槽。
- 滑块默认透明；鼠标进入结果区时显示半透明滑块。鼠标滚轮发生在结果区悬停状态，因此滚动时可见。
- 浅色模式使用深色半透明滑块，深色模式使用浅色半透明滑块；颜色只表达滚动位置，不改变选中项样式。
- 强制颜色模式不隐藏滑块，使用系统 `ButtonText` 颜色保证可发现性。
- 不添加 JavaScript 滚动监听、定时隐藏状态、React state、额外 DOM 或第三方滚动组件。

透明轨道让滚动条视觉上悬浮在结果区边缘。是否由 Windows/WebView2 以真正 overlay 方式绘制仍由平台决定；本任务不使用已弃用的 `overflow: overlay`，也不承诺零布局占位。即使平台保留滚动槽，其宽度也限制为 `6px`。
...
- 鼠标离开结果区后滑块恢复透明；键盘选择继续依赖选中行和 `nearest` 自动滚动，不为显示滑块转移焦点。
...
- 用聚焦测试确认滚动条宽度、默认透明、悬停显示和强制颜色回退均只作用于 `.result-list`。
