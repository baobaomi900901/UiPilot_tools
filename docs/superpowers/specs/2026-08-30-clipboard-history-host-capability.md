...
用户需要一个 Panel 类型的公开插件，在 UiPilot 运行期间持续记录最近 20 次剪贴板变化，包括：
...
6. Panel 存储只接受有限 JSON，且每插件配额为 5 MiB，不适合保存最多 100 MiB 的图片历史。
...
- 每插件最多保存 20 条，按宿主内部 recency 排名倒序排列。
...
- 图片由宿主转为 PNG 保存，单张最多 10 MiB，当前历史中的图片总量最多 100 MiB。
...
| Per-plugin history store | 管理 20 条环形历史、图片文件、revision、原子持久化与生命周期清理 |
...
- [ ] 单张图片和图片总量限制分别为 10 MiB 和 100 MiB；淘汰同时清理图片文件。
...
当以上三项全部完成时，用户可以在微信等外部窗口中打开插件，仅用 Tab、方向键和 Enter 选择最近 20 条文本、图片或文件记录并自动粘贴。
