...
5. 将锁内状态替换为 `SettingsState { value: candidate, current_is_valid: true }`。
...
5. current 移动成功后立即把锁内状态替换为 `{ value: candidate, current_is_valid: true }`，不再执行可能失败的操作。

第一次没有有效 current 时跳过 backup 更新，直接以 `MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH` 把 current temp 移到 current。若内存值来自有效 backup，原 backup 保持不变。该提交成功后 `current_is_valid` 必须变为 `true`；紧接着的第二次提交必须进入已有 current 分支，并把第一次提交的值写成 backup。
