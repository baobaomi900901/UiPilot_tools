...
新请求可以再次发布一条。Promise 决议后、请求被新请求淘汰、超时、取消、禁用、卸载、升级、reload
或 Runtime generation 替换后，旧 API 调用返回 `ExpiredRequestError`。
...
操作乱序到达前端时，前端只接受更高的十进制修订，不能用迟到快照清除新徽标。
...
返回 DTO 至少包含规范十进制字符串 `revision`、`unreadCount` 和消息数组。`unreadCount` 最大为 100，
可以安全作为前端整数使用。

消息提交、已读和清空后，宿主向 `main` 发出只含消息修订和未读数的受控事件。事件不是消息持久化的
完成条件；前端丢失事件时，下次读取摘要或打开消息页必须从持久化状态恢复。插件 Runtime、插件窗口、
`find` 窗口和其他 label 不能订阅消息正文或调用消息管理命令。
...
后返回成功，不等待用户看到、点击或关闭 Windows 通知。
...
允许正常换行显示，但不解释任何标记。插件仍安装且图标 URL 可用时使用现有 `PluginIcon`；卸载、旧
generation 或图标加载失败时使用默认插件图标。
...
原生通知由宿主适配器统一发送，插件不能直接访问 Tauri notification API。实现优先使用与当前 Tauri 2
版本兼容的官方通知插件，并通过通知动作回调路由到消息页。通知 payload 由宿主构造，只包含宿主目标和
不透明消息 ID；接收端不信任外部字符串来选择任意窗口、命令或 URL。

通知发送前检查操作系统权限。`denied` 或发送错误记录脱敏诊断，但 `publish()` 仍以持久化成功返回。
消息内容不得写入普通诊断日志。
...
自动化通过后，由用户在普通权限的 `npm run tauri dev` 中执行一次真实验收。任何需要用户点击、前台焦点
或观察系统 UI 的步骤必须提前通知用户；自动化和 Agent 绝不控制鼠标或键盘。

1. 安装或更新 `demo-win`，确认新增消息权限并授权。
2. 输入 `/demo-win hello` 并回车，确认子窗口显示预期 `Return text`。
3. 确认同时出现 Windows 通知、托盘短时闪烁和设置按钮未读徽标。
4. 在主窗口或子窗口可见时再次调用，确认通知仍显示且窗口不被消息到达抢焦点。
5. 点击一张通知，确认主窗口打开到“设置 -> 消息”，历史按最新优先显示且徽标清零。
...
4. 打开消息页只标记进入截止点前消息已读，并发新消息不会被误清。
5. 设置消息页、清空全部、持久化历史和插件卸载保留符合用户合同。
6. 设置按钮徽标准确显示未读数量并在 100 条时显示 `99+`。
7. 每条消息尝试显示 Windows 通知并触发 6 秒单计时器托盘闪烁。
8. 消息到达绝不抢焦点；用户点击通知才打开并聚焦设置消息页。
9. `demo-win` 的消息内容与子窗口 `Return text` 完全相同，消息失败时窗口不部分成功。
10. `demo-return`、`/find`、计算、浏览器搜索及现有窗口行为没有回归。
