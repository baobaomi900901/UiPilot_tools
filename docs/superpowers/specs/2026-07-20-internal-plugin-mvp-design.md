...
- `manifest` 必须精确等于宿主支持的整数 `1`。
- `id` 为 1 到 64 个 ASCII 小写字母、数字、点或连字符。
- `version` 和 `minHostVersion` 使用不含预发布后缀的 `major.minor.patch` 三段无符号 32 位整数。
- `minHostVersion` 高于当前宿主版本时禁用插件，不尝试降级协议。
- `runtime` 必须是插件根内的相对 `.html` 普通文件。
- `feature.id` 为 1 到 64 个 ASCII 小写字母、数字、点或连字符。
- `feature.trigger` 为 2 到 64 个 ASCII 小写字母、数字、斜杠或连字符，以 `/` 开头。
...
- 查询在阻塞工作线程中最多等待运行时就绪 500ms；未就绪时返回空结果且不累计运行时超时。
...
Rust 接收有效响应后，为每项生成不透明 `resultId`，并在现有结果注册表中保存插件 ID、请求 ID 和真实
`copyText` 负载。主 WebView 执行时仍只回传当前 `requestId + resultId`。新查询、窗口隐藏、
插件禁用或宿主退出立即使对应动作失效。
...
- `onQuery` 处理器必须同步调用 `publishResults`；处理器返回后再发布会被桥接拒绝，避免响应绑定到后续请求。
...
- 每次响应最多 20 项。
- `title` 最多 200 个 Unicode 标量值，`subtitle` 最多 500 个 Unicode 标量值。
...
- 顶层导航只接受无显式端口的插件自定义来源及 Windows WebView2 对应的本地映射来源。
- 自定义资源处理器只提供插件根内已验证的普通文件。
- 插件根到目标资源的每一级目录都拒绝符号链接和 Windows 重解析点。
...
| code | 固定 message |
| --- | --- |
| `pluginQueryFailed` | `plugin query failed` |
| `pluginPermissionDenied` | `plugin permission denied` |
| `clipboardWriteFailed` | `clipboard write failed` |

旧结果继续复用现有 `staleRequest` 和 `unknownResult`。启动阶段无效或不兼容的插件被静默跳过，不向尚未
打开的主窗口发送错误。每次超时都静默返回空结果；第三次超时同时禁用插件。
