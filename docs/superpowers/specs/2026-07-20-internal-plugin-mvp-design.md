...
2. 插件可以注册一个静态斜杠触发词，接收实时查询并返回最多 20 个宿主渲染结果。
3. 插件只能申请版本化、可枚举的宿主动作；首版唯一动作是 `clipboard.writeText`。
4. 宿主保持对路由、结果时效、动作执行、权限、窗口和错误展示的最终控制。
5. 插件损坏、超时、崩溃或越权不能破坏应用搜索、主窗口或现有本地数据。
6. 用 `math` 插件完成一条可自动验收的端到端链路。
