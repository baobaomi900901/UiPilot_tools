...
function isSessionError(error) {
  return error?.code === 'ExpiredWindowSessionError'
}
...
function closePluginWindow() {
  void window.uipilotPluginWindow.close().catch(() => {})
}
...
function tryCopyFromList(event) {
  ...
  copyEditorContent({ onSuccess: closePluginWindow })
  return true
}
...
async function handleNavigationKeys(event) {
  ...
  event.preventDefault()
  await moveListSelection(event.key === 'ArrowDown' ? 1 : -1, { focusList: !fromSearch })
}
...
searchInput.addEventListener('input', () => {
  ...
  renderList()
  scrollToVisibleIndex(getVisibleIndex())
})
...
window.uipilotPluginWindow.onUpdate(async (update) => {
  ...
  sessionToken += 1
  const token = sessionToken
  storage = window.uipilotPluginWindow.storage
  searchQuery = searchInput.value
